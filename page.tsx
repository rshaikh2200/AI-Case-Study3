...
export default function DashboardPage() {
    ...
    async function fetchData() {
      try {
        const [sessionSnapshot, workflowSnapshot, userProfileSnapshot] = await Promise.all([
          getDocs(collection(db, 'session table')),
          getDocs(collection(db, 'workflowData')),
  ...
  const calculateAvgTime = () => {
    ...
    const completionTimes = sessionData.map(session => {
      ...
      const lastWorkflow = sessionWorkflows.sort((a, b) =>
        b.nextButtonTimestamp.seconds - a.nextButtonTimestamp.seconds
      )[0];
      if (!lastWorkflow) return 0;
      const endTime = new Date(lastWorkflow.nextButtonTimestamp.seconds * 1000);
      return (endTime - startTime) / (1000 * 60);
    }).filter(time => time > 0);
    return Math.round(completionTimes.reduce((acc, time) => acc + time, 0) / completionTimes.length);
  };
...
const calculateAvgTimePerQuestion = () => {
  ...
  const totalTimeInSeconds = workflowData.reduce((acc, w) => {
    ...
    if (w.attempt1Timestamp && w.nextButtonTimestamp) {
      // Convert Firestore Timestamp to Date object and calculate the time difference in seconds
      const startTime = w.attempt1Timestamp.toDate(); // Converts Firestore timestamp to JavaScript Date
      const endTime = w.nextButtonTimestamp.toDate(); // Same for nextButtonTimestamp
...
const calculateHighPerformers = () => {
  ...
  const highPerformers = userProfileData.filter(user => {
    ...
    const successRate = (userWorkflows.filter(w => w.attempt1Result === 'Correct').length / userWorkflows.length) * 100;
...
const calculateFastestCompletionTime = () => {
  ...
  const times = sessionData.map(session => {
    const sessionWorkflows = workflowData.filter(w => w.sessionID === session.sessionID);
    if (!sessionWorkflows.length) return null;
    const start = new Date(session.startTime.seconds * 1000);
    const end = new Date(sessionWorkflows[sessionWorkflows.length - 1].nextButtonTimestamp.seconds * 1000);
...
const calculateFailedAfterSecondAttempt = () => {
  ...
  return Math.round((failedSecondAttempts / secondAttempts.length) * 100);
};
...
<div className="bg-white rounded-lg shadow p-6">
  ...
  <div className="space-y-4">
    ...
    <div className="flex justify-between items-center">
      ...
      <span className="text-2xl font-bold text-[#45B7D1]">{calculateFirstAttemptSuccessRate()}%</span>
    </div>
    <div className="flex justify-between items-center">
      <span className="text-gray-600">Second Attempt Success Rate</span>
      <span className="text-2xl font-bold text-[#45B7D1]">{calculateSecondAttemptSuccessRate()}%</span>
    </div>

    <div className="flex justify-between items-center">
      <span className="text-gray-600">Failed After Second Attempt</span>
      <span className="text-2xl font-bold text-[#96CEB4]">{calculateFailedAfterSecondAttempt()}%</span>
    </div>
    ...
    <div className="flex justify-between items-center">
      ...
      <span className="text-2xl font-bold text-[#9333EA]">{Math.round(workflowData.length / totalSessions)}</span>
    </div>

    {/* New Metrics */}
    <div className="flex justify-between items-center">
      <span className="text-gray-600">Average Time per Question</span>
      <span className="text-2xl font-bold text-[#45B7D1]">{calculateAvgTimePerQuestion()} sec</span>
    </div>
    <div className="flex justify-between items-center">
      <span className="text-gray-600">Completion Rate</span>
      <span className="text-2xl font-bold text-[#96CEB4]">{calculateCompletionRate()}%</span>
    </div>
    <div className="flex justify-between items-center">
      <span className="text-gray-600">High Performers</span>
      <span className="text-2xl font-bold text-[#F86CCF]">{calculateHighPerformers()}%</span>
    </div>

    <div className="flex justify-between items-center">
      <span className="text-gray-600">Fastest Completion Time</span>
      <span className="text-2xl font-bold text-[#F86CCF]">{calculateFastestCompletionTime()}m</span>
    </div>
    <div className="flex justify-between items-center">
      <span className="text-gray-600">Median Time to Complete Training</span>
      <span className="text-2xl font-bold text-[#9333EA]">{calculateMedianCompletionTime()}m</span>
    </div>
  </div>
</div>
