...
export async function POST(request) {
  try {
    ...
    - A medical case study for a ${sanitizedRole} in the ${sanitizedDepartment} department specializing in ${sanitizedSpecialization}.  Each case study should include a  medical error (ex: a instrument left within patient, missing items, bioburden, burn events, isntrument malfunction, shift change, site markings, consent, miscoummunication) that occured, however it should not include what steps were taken to resolve the issue by team or individual only provide the scenario. The case studies should incorporate health equity with diverse name, and gender.  
...
- **Avoid NSFW Content**: Ensure that the prompt does not contain or imply any inappropriate, offensive, nudity or unsafe content.
