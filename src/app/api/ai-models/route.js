...
export async function POST(request) {
  try {
    ...
    - A medical case study for a ${sanitizedRole} in the ${sanitizedDepartment} department specializing in ${sanitizedSpecialization}. Each medical case study should focus on a different medica error error: Retained Sponge, Missing Items on tray (equipment missing, broken, or not available), bioburden on instrument, Bovey (burn events), Fires where the patient was bruned (oxygen being introduced and then a spark from instrument, sets it off, a. Didnt have right oxygen mask which caused oxygen leak), Patient falls, speciment error (a. not labeled correctly, b. discarded by accident, c. lost speciment, speciment handoff), shift change ( when the count process is happening, when the procedure is almost done, surgeon is not aware of shift change), site markings (wrong site procedure, limbs are okay but moreso on inside the body organs), consent (consent form is missing).
